...
void useDefensiveItem(Character* character, Item& item)
{
    ...
    if( auto* c1 = dynamic_cast<Dwarf*>(character) )
    {
        c1->boostArmor( item.getBoost() * 1.1 );
    }
    else if( auto* c2 = dynamic_cast<Paladin*>(character) )
    {
        //same with paladins
        c2->boostArmor( item.getBoost() * 1.3 );
    }
    else if( auto* c3 = dynamic_cast<DragonSlayer*>(character))
    {
        c3->boostArmor( item.getBoost() * 1.5 );
    }
    else if( auto* c4 = dynamic_cast<Dragon*>(character) )
    {
...
void useHelpfulItem(Character* character, Item* item)
{
    if( auto* c1 = dynamic_cast<Dwarf*>(character) )
    {
        c1->boostHitPoints(item->getBoost() * 2);
    }
    else if( auto* c2 = dynamic_cast<Paladin*>(character) )
    {
        c2->boostHitPoints(item->getBoost() * 1.5);
    }
    else if( auto* c3 = dynamic_cast<DragonSlayer*>(character))
    {
        c3->boostHitPoints(item->getBoost() * 1.25);
    }
    else if( auto* c4 = dynamic_cast<Dragon*>(character) )
    {
...
void useAttackItem(Character* character, Item* item)
{
    if( auto* c1 = dynamic_cast<Dwarf*>(character) )
    {
        c1->boostAttackDamage(item->getBoost() * 1.5);
    }
    else if( auto* c2 = dynamic_cast<Paladin*>(character) )
    {
        c2->boostAttackDamage(item->getBoost() * 1.33);
    }
    else if( auto* c3 = dynamic_cast<DragonSlayer*>(character))
    {
        ...
        c3->boostAttackDamage(c3->getAttackDamage() * item->getBoost());
    }
    else if( auto* c4 = dynamic_cast<Dragon*>(character) )
    {
